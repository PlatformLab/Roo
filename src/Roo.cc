...
{
    request->reserve(sizeof(Proto::Message::Header));
}
...
void
RemoteOp::send(Homa::Driver::Address destination)
{
    state.store(State::IN_PROGRESS);
    Homa::Driver::Address replyAddress = transport->driver->getLocalAddress();
    SpinLock::Lock lock_transport(transport->members->mutex);
    opId = Proto::OpId(transport->members->transportId,
                       transport->members->nextOpSequenceNumber++);
    Proto::Message::Header outboundHeader(opId,
...
{
    if (request != nullptr) {
        Proto::Message::Header header(opId,
                                      Proto::Message::ULTIMATE_RESPONSE_ID);
        transport->driver->addressToWireFormat(replyAddress,
                                               &header.replyAddress);
...
void
ServerOp::delegate(Homa::Driver::Address destination)
{
    if (request != nullptr) {
        delegated = true;
        Proto::Message::Header header(opId, stageId + 1);
        transport->driver->addressToWireFormat(replyAddress,
                                               &header.replyAddress);
        response->prepend(&header, sizeof(header));
...
OpManager::OpManager(Homa::Driver* driver, uint64_t transportId)
    : driver(driver)
    , internal(Homa::Transport::create(driver, transportId))
...
OpManager::receiveServerOp()
{
    ...
    if (!members->pendingServerOps.empty()) {
        ...
        op.response->reserve(sizeof(Proto::Message::Header));
        op.transport = this;
...
OpManager::poll()
{
    ...
    for (Homa::InMessage* message = internal->receive(); message != nullptr;
