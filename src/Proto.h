...
struct BranchId {
    ...
    uint64_t sequence;  ///< Uniquely identifies the branch within the task.
    ...
    BranchId(TaskId taskId, uint64_t sequence)
        : taskId(taskId)
        , sequence(sequence)
    {}
    ...
    struct Hasher {
        ...
        std::size_t operator()(const BranchId& branchId) const
        {
            ...
            std::size_t h2 = std::hash<uint64_t>()(branchId.sequence);
            return h1 ^ (h2 << 1);
        }
    };
} __attribute__((packed));
...
struct Manifest {
    ...
    uint64_t requestCount;   ///< Number of requests generated by this task.
    uint64_t responseCount;  ///< Number of responses sent by this task.
